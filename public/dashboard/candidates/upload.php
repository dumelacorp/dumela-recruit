...
debug_log("Processing upload for email: " . $email);
...
try {
    ...
    if($_SERVER['REQUEST_METHOD'] == 'POST') {
        if(isset($_POST['candidate_action']) && $_POST['candidate_action'] == 'create') {
            ...
            $candidate->outsource_rate = $_POST['rate']; // Use the same rate
            $candidate->outsource_rate_period = $_POST['rate_period'];
            ...
            if (isset($_FILES['resume']) && $_FILES['resume']['error'] == 0) {
                ...
                debug_log("Attempting to upload file to: " . $target_file);

                if (move_uploaded_file($_FILES['resume']['tmp_name'], $target_file)) {
                    $candidate->resume = $unique_filename;
                    debug_log("File uploaded successfully");

                    try {
                        if($candidate->create()) {
                            debug_log("Candidate created successfully");
                            $_SESSION['message'] = "Data uploaded successfully! Please login to continue.";
                        ...
                        debug_log("Error creating candidate: " . $e->getMessage());
                        $_SESSION['alert']['type'] = 'error';
                        $_SESSION['alert']['message'] = "Failed to create profile. Please try again.";
                    }
                } else {
                    debug_log("File upload failed");
                    $_SESSION['alert']['type'] = 'error';
                    $_SESSION['alert']['message'] = "Failed to upload resume. Please try again.";
                }
            } else {
                debug_log("No file uploaded or file upload error");
                $_SESSION['alert']['type'] = 'error';
                $_SESSION['alert']['message'] = "Please select a resume file.";
            }
        }
    }
} catch(Exception $e) {
    ...
    $_SESSION['alert']['type'] = 'error';
    $_SESSION['alert']['message'] = "An unexpected error occurred. Please try again.";
}
?>
